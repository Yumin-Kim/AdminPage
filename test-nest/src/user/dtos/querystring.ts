...
export function parseQueryString<J extends T, T>(
  queryString: T,
  parseColumn: string[],
  mergeObj: J,
): J {
  (Object.keys(queryString) as [keyof T]).map((value) => {
    return parseColumn.map((member) => {
      if (value === member) {
        (mergeObj[value] as J[keyof J]) = queryString[member];
        if (value === 'host') {
          return ((mergeObj[value] as any) =
            (mergeObj[value] as any) === 'true' ? true : false);
        }
        return mergeObj[value];
      }
    });
  });
  return mergeObj;
...
export function filterGroup_GroupByObject<T>(
  queryStringObj: T,
  DesireArray: (keyof T)[],
) {
