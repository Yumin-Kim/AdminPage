...
import {
  ParkingInfos,
  UserCarInfos,
} from 'src/usercar/entities/usercar.entity';
...
export class Group {
  ...
  @OneToMany(
    ...
    (admindashboard) => admindashboard.group,
  )
...
export class GroupByGroups {
  ...
  @OneToMany(
    (type) => AccessMembers,
    (accessmembers) => accessmembers.groupbygroups,
  )
  accessmembers: AccessMembers[];
  ...
  @OneToMany(
    (type) => UserCarInfos,
    (UserCarInfos) => UserCarInfos.groupbygroup,
  )
  UserCarInfos: UserCarInfos[];

  @OneToMany(
    (type) => ParkingInfos,
    (parkingInfos) => parkingInfos.groupbygroup,
  )
  parkingInfos: ParkingInfos[];

  @OneToMany(
    (type) => PricingInfos,
    (pricingInfos) => pricingInfos.groupbygroup,
  )
  pricingInfos: PricingInfos[];
  ...
  @OneToMany(
    (type) => FacilityInfos,
    (facilityInfos) => facilityInfos.groupbygroup,
  )
  facilityInfos: FacilityInfos[];
}
@Entity('users')
...
export class AccessMembers {
  ...
  @ManyToOne(
    (type) => GroupByGroups,
    (groupbygroups) => groupbygroups.accessmembers,
  )
  groupbygroups: GroupByGroups;
